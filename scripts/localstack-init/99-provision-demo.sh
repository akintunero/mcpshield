#!/bin/bash
# LocalStack init script — runs every time LocalStack starts.
# Provisions 57+ vulnerable resources for MCPShield demo.
set -euo pipefail

AWS() { aws --endpoint-url=http://localhost:4566 --region=us-east-1 "$@"; }
try() { AWS "$@" 2>/dev/null && echo "  ✅ $*" || true; }

echo "━━━ MCPShield: Provisioning demo vulnerable resources ━━━"
...
echo "━━━ MCPShield demo resources provisioned ━━━"
