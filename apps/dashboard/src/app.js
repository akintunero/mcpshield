...
const FINDINGS_CATALOG = {
  'MCPS-S3-001': {
    businessImpact: 'Sensitive customer or corporate data can be exfiltrated by anyone, leading to breach notifications, regulatory fines, and brand loss.',
    technicalImpact: 'Anonymous principals can list and download objects, enabling data theft, tampering, or malicious content hosting.',
    attackScenario: 'An attacker scans for open buckets, locates bucket name, and downloads private spreadsheet files without authentication.',
    bestPractice: 'Enable S3 Block Public Access at the bucket level and remove any public policies or ACLs.',
  ...
  'MCPS-IAM-001': {
    businessImpact: 'Compromise of this user credential yields full control of the cloud account, enabling data deletion, ransomware, and billing abuse.',
    technicalImpact: 'The user can perform any action on any resource, including deleting logs, disabling CloudTrail, and creating backdoor users.',
    attackScenario: 'An access key for this user leaks in a git commit. The attacker uses it to spin up GPU miners and delete production databases.',
    bestPractice: 'Manage access using groups or roles. Never attach policies directly to individual IAM users.',
...
# }`,
  ...
  'MCPS-IAM-002': {
    businessImpact: 'Long-lived credentials are more likely to leak over time (laptops, backups, configurations), presenting a permanent access risk.',
    technicalImpact: 'Active credentials remain valid indefinitely, exposing the account if the key has been leaked or cached.',
    attackScenario: 'An engineer leaves the company but retains local credentials that were never rotated; they are used to access database snapshots.',
    bestPractice: 'Enforce access key rotation policies of 90 days or less. Prefer temporary role credentials.',
...
aws --endpoint-url {{endpoint}} iam create-access-key --user-name {{userName}}`
  ...
  'MCPS-EC2-001': {
    businessImpact: 'Exposing management ports to the world invites constant automated brute-force attacks and scans for unpatched exploits.',
    technicalImpact: 'Allows anyone on the internet to attempt SSH authentication, creating code execution opportunities if SSH daemon is vulnerable.',
    attackScenario: 'A botnet scans for open port 22, performs credential brute-forcing, gains shell access, and installs malware.',
    bestPractice: 'Restrain SSH access to known corporate CIDR blocks or use AWS Systems Manager Session Manager.',
  ...
  'MCPS-EC2-002': {
    businessImpact: 'Public RDP servers are prime targets for ransomware operators who use compromised endpoints to lock network drives.',
    technicalImpact: 'Exposes Windows login interface to internet scanners, increasing risk of brute-forcing or BlueKeep style RCE.',
    attackScenario: 'An attacker detects port 3389, runs an RDP exploit to bypass authentication, and deploys locker payloads.',
    bestPractice: 'Restrain port 3389 ingress to trusted IP ranges or run behind a Client VPN / Bastion host.',
  ...
  'MCPS-S3-002': {
    businessImpact: 'Failing to encrypt data at rest violates core standards (HIPAA/PCI DSS) and risks regulatory fines if physical disks are breached.',
    technicalImpact: 'Objects are written to storage in plaintext. Access to the underlying physical layers exposes all corporate files.',
    bestPractice: 'Configure default bucket server-side encryption to AES256 or use AWS KMS customer keys.',
  ...
  'MCPS-S3-003': {
    businessImpact: 'Without versioning, objects accidentally deleted or overwritten by application bugs are permanently unrecoverable.',
    technicalImpact: 'A delete request permanently destroys the targeted object version; recovery requires restored backups.',
    bestPractice: 'Enable bucket versioning to ensure deleted or overwritten objects can be recovered from version history.',
  ...
  'MCPS-CT-001': {
    businessImpact: 'Without audit logs, security responders cannot trace when, who, or how a resource was modified during a breach.',
    technicalImpact: 'Removes the primary forensic and log trail of API activities in the AWS account.',
    bestPractice: 'Configure at least one multi-region CloudTrail, delivering logs to a dedicated S3 bucket.',
...
aws --endpoint-url {{endpoint}} cloudtrail start-logging --name mcpshield-trail`
  ...
  'MCPS-IAM-003': {
    businessImpact: 'Weak password policies invite dictionary attacks against the AWS console, potentially leading to administrative takeover.',
    technicalImpact: 'Allows users to configure simple, short, or reusable passwords, lowering authorization resistance.',
    bestPractice: 'Configure an account password policy enforcing a minimum length of 14, complexity, and reuse prevention.',
  ...
  'MCPS-IAM-004': {
    businessImpact: 'Forgotten, inactive identities represent a static attack surface that is rarely audited, making them ideal targets.',
    technicalImpact: 'The identity retains programmatic access keys or console passwords that can be abused without notice.',
    bestPractice: 'Disable or delete users that have not authenticated or used credentials within the last 90 days.',
...
aws --endpoint-url {{endpoint}} iam delete-user --user-name {{userName}}`
  ...
  'MCPS-IAM-005': {
    businessImpact: 'Active, unused programmatic access keys represent a standing risk that can be leaked through backups or compromised files.',
    technicalImpact: 'An active credentials key remains capable of authorizing programmatic requests to the account.',
    bestPractice: 'Deactivate and delete any IAM access keys that have not recorded usage in the last 90 days.',
...
# aws iam update-access-key --user-name {{userName}} --access-key-id {{accessKeyId}} --status Inactive`,
  ...
  'MCPS-S3-004': {
    businessImpact: 'Failing to record read/write requests on storage buckets makes list and download operations invisible, disabling exfiltration forensics.',
    technicalImpact: 'API calls to download objects do not emit logs to server access logs.',
    bestPractice: 'Configure S3 Server Access Logging to deliver request logs to a dedicated logging bucket.',
  ...
  'MCPS-TAG-001': {
    businessImpact: 'Untagged resources cause accounting blindspots, complicate compliance audits, and prevent automated security response rules.',
    technicalImpact: 'Security automation guardrails cannot identify owner, classification, or environment, bypassing filters.',
    bestPractice: 'Enforce tags (Owner, Environment, DataClassification) on all provisioned infrastructure.',
...
# tags = { Owner = "security", Environment = "workshop", DataClassification = "internal" }`,
  ...
  'MCPS-NAM-001': {
    businessImpact: 'Non-conforming resource names slow operations, increase administrative errors, and violate internal compliance guidelines.',
...
aws --endpoint-url {{endpoint}} s3 sync s3://{{bucket}} s3://workshop-mcpshield-data`
  ...
  'MCPS-DESC-001': {
    businessImpact: 'Missing descriptions slow down review cycles and prevent engineers from identifying the original intent of a resource.',
    technicalImpact: 'Auditors cannot quickly map security group rules or configuration variables to their business goals.',
    bestPractice: 'Provide descriptive metadata on all Security Groups and Parameter configurations.',
...
# description = "Managed by MCPShield — {{resourceType}} for workshop"`,
  ...
  'MCPS-SSM-001': {
    businessImpact: 'Unencrypted parameters are readable in plain-text by any user with read permissions, leading to key leakage and account compromise.',
    technicalImpact: 'Plaintext storage allows easy extraction of API keys, database credentials, and certificates.',
  ...
  'MCPS-LM-001': {
    ...
    technicalImpact: 'Deprecated runtimes miss critical security patches and performance optimizations.',
    attackScenario: 'An attacker exploits an unpatched runtime vulnerability on a deprecated engine version to escape the Lambda sandbox.',
  ...
  'MCPS-SQS-001': {
    ...
    technicalImpact: 'Plaintext drive exposure could allow unauthorized AWS identities to read message contents directly.',
  ...
  'MCPS-SEC-001': {
    businessImpact: 'Any user with Secrets Manager read access can read the secret value without requiring separate KMS decryption rights.',
...
function selectFinding(finding) {
  ...
  document.getElementById('detail-resource').textContent = `${finding.resource.service}::${finding.resource.type}/${finding.resource.id}`;
...
async function syncState(forceScan = false) {
  ...
  try {
    ...
    document.getElementById('resources-count').textContent = data.lastScan ? data.lastScan.resourcesScanned : 0;
    
    if (data.lastScan && data.lastScan.completedAt) {
      const date = new Date(data.lastScan.completedAt);
      document.getElementById('last-scan-time').textContent = date.toLocaleTimeString() + ' ' + date.toLocaleDateString();
    } else {
    ...
    const max = Math.max(breakdown.critical, breakdown.high, breakdown.medium, breakdown.low, resolvedCount, 1);
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  async function sendChatMessage() {
    ...
    try {
      ...
      if (query.includes('scan') || query.includes('fix') || query.includes('approve') || query.includes('remediat')) {
        syncState(false);
      }
    } catch (err) {
      typingEl.remove();
      appendMessage('assistant error', 'Encountered an error communicating with the analyst agent.');
      console.error(err);
    } finally {
